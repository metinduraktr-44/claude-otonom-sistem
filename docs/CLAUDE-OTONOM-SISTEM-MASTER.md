# CLAUDE OTONOM SİSTEM — TEK PARÇA MASTER DOKÜMAN
## (Cilt 1 + Cilt 2 + Cilt 3 + Cilt 4 — birleşik, tam hâliyle)
**Hedefler:** `https://claude.ai/cowork/cse_0162sfWdp6sY4Z2E4WSxQE1f` (Cowork projesi) · `https://github.com/metinduraktr-44/claude-otonom-sistem` (GitHub)
**Kapsam:** aitmpl.com'un 8 bileşen tipi (Skills/Agents/Commands/Settings/Hooks/MCPs/Loops/Plugins) — anatomi + üretim + doğrulanmış en yüksek yıldızlı kaynaklar + güvenlik/verimlilik çerçevesi + Cowork kurulum talimatı, tek belgede.

## İÇİNDEKİLER
- **BÖLÜM A** — Proje Anayasası: gerçeklik kontrolü, aitmpl.com anatomisi, tersine mühendislik rehberi, Chat/Code/Cowork mimarisi, ANA MASTER PROMPT, gecelik takvim, zaman damgası protokolü, gelir motoru
- **BÖLÜM B** — Bileşen Prompt Kütüphanesi: 8 kategorinin her biri için repo-doğrulanmış üretim + gecelik iyileştirme prompt'u, global ajan gelir mekanizması
- **BÖLÜM C** — GitHub Ekosistem Haritası: her kategori için en yüksek yıldızlı depolar, "top 6 ekip" sentezi, GitHub-dışı doğrulama kaynakları (OpenSSF Scorecard, akademik güvenlik verisi), kategori bazlı en güvenli seçim tablosu
- **BÖLÜM D** — Cowork Master Talimatı: kopyala-yapıştır operasyon anayasası, güvenlik + verimlilik çerçevesi, GitHub senkronizasyon haritası

> NOT (yerleşim): BÖLÜM A = docs/CILT1-PROJE-ANAYASASI.md · BÖLÜM B = docs/CILT2-BILESEN-PROMPT-KUTUPHANESI.md (bu ikisi repoda tam hâlleriyle zaten mevcut — verimlilik ilkesi gereği burada tekrarlanmaz, oradan okunur). BÖLÜM C ve D'nin tam metni aşağıda ve ayrıca docs/CILT3-GITHUB-EKOSISTEM-HARITASI.md + docs/CILT4-COWORK-MASTER-TALIMATI.md dosyalarındadır.
...
---
# KAPANIŞ — TEK BELGE OLARAK KULLANIM
**Cowork'e kurulum:** BÖLÜM D içeriği Cowork projesinin Instructions alanına yapıştırılır — A/B/C'yi referans veren özet-operasyonel versiyondur; günlük çalışma onunla yürür.
**GitHub'a kayıt:** Bu belge `docs/CLAUDE-OTONOM-SISTEM-MASTER.md`; BÖLÜM A/B/C/D ayrıca `docs/CILT1..4` olarak ayrı dosyalarda.
**Gelir motoru:** K-001 ile CLAUDE OTONOM SİSTEM'in kendisine bağlı; 5 pilot iş (Tahmin Uzmanı, VESTRA, İBB Kültür AŞ, Movéa, Response DGA) dogfooding — detay: GELIR_MOTORU.md. Bu çerçeve BÜTÜN repolara uygulanır.
